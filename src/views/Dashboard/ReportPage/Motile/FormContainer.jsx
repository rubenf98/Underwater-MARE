...
function FormContainer(props) {
    ...
    const handleOk = () => {
        form.validateFields().then(values => {
            if (current.id) {
                props.update(current.id, { ...values, project_id: projectId, taxa_id: values.taxa_id[1] }).then(() => {
                    handleCancel();
                });
            } else {
                props.create({ ...values, project_id: projectId, taxa_id: values.taxa_id[1] }).then(() => {
                    handleCancel();
                });
            }
        ...
        >
            ...
            <Form style={{ margin: "30px auto" }} layout="vertical" requiredMark form={form}>
                <Row gutter={16}>
                    ...
                    <Col xs={24} md={6}>
                        <Form.Item label="Surveyed area" name="surveyed_area" rules={requiredRule}>
                            <InputNumber style={{ width: "100%" }} />
                        </Form.Item>
                    </Col>

                    <Col xs={24} md={6}>
                        <Form.Item label="Density/100" name="density/100" rules={requiredRule}>
                            <InputNumber style={{ width: "100%" }} />
                        </Form.Item>
                    </Col>
                    <Col xs={24} md={6}>
